...
async function requireManifest(projectsRoot) {
     ...
     * @type {Boolean}
     */
    const hasIndexDir = await findFile(projectsRoot, 'index.js').catch((err) => {
        logger.error(err);
     ...
     * @type {Boolean}
     */
    const hasPackageDir = await findFile(projectsRoot, 'package.json').catch((err) => {
        logger.error(err);
    });

    for (const indexDir of hasIndexDir) {
        // 加载内建业务逻辑
        try {
            // 加载业务的index文件
            // eslint 不希望动态加载代码
            ...
            const indexModuleType = typeof businessIndex;

            // eslint 认为字面量最好在右面
            // 但是在左侧可以防止变成意外的赋值，字符在左侧赋值会报错的
            if ('object' === indexModuleType) {
                ...
                // 获取业务名称
                const tempArr = indexDir.split('/');
                const businessName = tempArr[tempArr.length - 2];

                // businessIndex 是变量但是有可能是构造函数
                const businessObj = new businessIndex();
    ...
    const hasRouterDir = await findFile(projectsRoot, 'router');

    for (const routerDir of hasRouterDir) {
        try {
